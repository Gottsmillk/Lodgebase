...
public class User 
{
	...
	public void updateRoomArray(String numGuests, String numBeds, String check_in, String check_out, String smokingYOrN, String suiteYOrN, String minPrice, String maxPrice) throws ClassNotFoundException, SQLException
	{
		roomArray.clear();
		Class.forName("com.mysql.jdbc.Driver");
		conn=DriverManager.getConnection("jdbc:mysql://lodgebasedb.ctud3vauv27c.us-east-1.rds.amazonaws.com/bookingproject","lodgebase","lodgebase");
		if(smokingYOrN.equalsIgnoreCase("smoking")){
			String smokingParsed = "y";
		}
		else{
			String smokingParsed = "n";
		}
		if(suiteYOrN.equalsIgnoreCase("suite")){
			String suiteParsed = "y";
		}
		else{
			String suiteParsed = "n";
		}
		String query = "SELECT * FROM bookingproject.rooms WHERE numberOfPeople >= '" + numGuests + "' AND numberOfBeds >= '" + numBeds + "' AND price >= '" + minPrice + "' AND price <= '" + maxPrice + "'";
		...
		while(rs.next()){
			...
			tempRoom.price = rs.getInt("price");
			tempRoom.roomNumber = rs.getInt("roomNumber");
			if(rs.getString("smoking") == "y"){
				tempRoom.smoking = true;
			}
			else{
				tempRoom.smoking = false;
			}
			if(rs.getString("suite") == "y"){
				tempRoom.suite = true;
			}
			else{
				tempRoom.suite = false;
			}
			String query2 = "SELECT * FROM bookingproject.userhistory WHERE roomid = '" + rs.getInt("roomid") + "' AND ((check_in <= '" + check_in + "' AND check_out >= '" + check_in + "') OR (check_in <= '" + check_out + "' AND check_out >= '" + check_out + "'))";
			...
			if(!rs2.next()){
				...
				System.out.println("match not found for: " + rs.getInt("roomid"));
			}
		}
	}
